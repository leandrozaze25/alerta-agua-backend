...
# !!! IMPORTANTE: Substitui este token pelo token REAL do teu dispositivo S20 que viste no Logcat !!!
...
try:
    ...
    # Evita erro se o SDK já foi inicializado por uma execução anterior
...
def buscar_e_guardar_dados_sanepar():
    ...
    try:
        ...
        # Verifica se houve mudança antes de guardar para enviar notificação
        ...
        else: # Primeira vez que executamos
            houve_mudanca = True
            print(f"!!! MUDANÇA DETETADA: Primeiro status recebido: '{mensagem_nova}' !!!")
            
        # Guarda os novos dados e o timestamp no Redis
        ...
        # Se houve mudança (e não era a primeira execução), envia a notificação
        if houve_mudanca and status_antigo_json:
