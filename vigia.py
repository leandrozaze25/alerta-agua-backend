...
try:
    ...
    exit() # Se não conseguirmos conectar ao Redis, paramos o script.

def enviar_notificacao_fcm(titulo, corpo):
    # ... (código da função de notificação permanece igual) ...
...
def verificar_sanepar():
    ...
    try:
        ...
        novo_status_json = resposta.text
        ...
        if not status_antigo_json or mensagem_nova != mensagem_antiga:
            print(f"!!! MUDANÇA DETETADA: De '{mensagem_antiga or 'Nenhum'}' para '{mensagem_nova}' !!!")
            cliente_redis.set(REDIS_CHAVE_STATUS, novo_status_json)
            print("VIGIA_REDIS: Novo status guardado no Redis.")
            if status_antigo_json: # Só envia notificação se não for a primeira vez
...
if __name__ == '__main__':
    verificar_sanepar()
    print("VIGIA: Verificação única concluída.")
